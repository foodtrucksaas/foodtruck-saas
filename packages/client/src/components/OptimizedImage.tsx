...
function generateSrcSet(src: string, widths: number[]): string {
  ...
  // For now, we provide the original image at different densities
  if (src.includes('supabase.co/storage')) {
    // Supabase storage URLs support width transforms
    return widths
      .map((w) => {
  ...
  // For other URLs, return original (browsers will handle scaling)
  return src;
}
...
function addWidthToSupabaseUrl(url: string, width: number): string {
  ...
  // Format: ?width=300&resize=contain
  const separator = url.includes('?') ? '&' : '?';
  return `${url}${separator}width=${width}&resize=contain`;
}

// Generate placeholder data URL (tiny colored rectangle)
...
export interface OptimizedImageProps {
  ...
  alt: string;
  /** Width of the image (helps prevent CLS) */
  width?: number | string;
  /** Height of the image (helps prevent CLS) */
 ...
 * - Layout shift prevention (width/height)
 */
...
export function useImagePreload(urls: string[]): boolean {
  ...
  useEffect(() => {
    ...
    Promise.all(urls.filter(Boolean).map(preloadImage))
      ...
      .catch(() => setAllLoaded(true)); // Mark as loaded even on error
  }, [urls]);
