-- Optimized offer calculation with global optimization strategy
-- Key principles:
-- 1. For bundles (fixed price): use MOST EXPENSIVE items to maximize savings
-- 2. For buy_x_get_y: FREE item = CHEAPEST among eligible
-- 3. When offers compete for items, find optimal allocation
...
DECLARE
  ...
  v_best_bundle_discount INTEGER;
  v_best_bundle_id UUID;
  v_best_bundle_name TEXT;
  v_best_bundle_items JSONB;
  v_remaining_items JSONB;
  v_item RECORD;
  v_sorted_items JSONB;
  v_items_for_offer JSONB;
...
BEGIN
  ...
  -- Initialize remaining items from cart (expanded by quantity)
  ...
  FOR v_offer IN
    ...
    ORDER BY COALESCE((o.config->>'fixed_price')::INTEGER, 0) DESC -- Process expensive bundles first
  LOOP
    ...
    IF v_config->'bundle_categories' IS NOT NULL THEN
      -- Category-based bundle: check if we have items from each required category
      DECLARE
        v_bundle_cat RECORD;
      ...
      BEGIN
        ...
        LOOP
          -- Find most expensive items in this category from remaining items
          DECLARE
            v_cat_items RECORD;
        ...
        IF v_can_apply THEN
          ...
          IF v_bundle_savings > 0 THEN
            ...
            FOR v_item IN SELECT * FROM jsonb_array_elements(v_consumed_items)
            LOOP
              UPDATE temp_remaining_items
              SET quantity = quantity - (v_item.value->>'quantity')::INTEGER
              WHERE menu_item_id = (v_item.value->>'menu_item_id')::UUID;
            END LOOP;
            ...
            INSERT INTO temp_optimized_offers VALUES (
              ...
              NULL, v_bundle_savings::DECIMAL, 1 -- High priority
            );
          END IF;
        END IF;
      END;
    ELSE
      -- Specific items bundle (legacy)
      ...
      WHERE oi.offer_id = v_offer.id
        ...
        AND EXISTS (
          SELECT 1 FROM temp_remaining_items tri
          WHERE tri.menu_item_id = oi.menu_item_id
            AND tri.quantity >= oi.quantity
        );
      ...
      IF v_total_items >= v_bundle_items_count THEN
        ...
        IF v_discount_per_app > 0 THEN
          -- Consume items
          UPDATE temp_remaining_items tri
          SET quantity = quantity - COALESCE((
            SELECT oi.quantity FROM offer_items oi
            WHERE oi.offer_id = v_offer.id
              AND oi.menu_item_id = tri.menu_item_id
              AND oi.role = 'bundle_item'
          ), 0)
          ...
          INSERT INTO temp_optimized_offers VALUES (
            ...
            NULL, v_discount_per_app::DECIMAL / GREATEST(v_bundle_items_count, 1), 1
          );
        END IF;
      END IF;
    END IF;
  ...
  LOOP
    ...
    IF (v_config->>'type') = 'category_choice' AND v_config->'trigger_category_ids' IS NOT NULL THEN
      ...
      IF v_has_overlap THEN
        ...
        -- Items needed per application = trigger_qty + reward_qty
        ...
        IF v_max_applications > 0 THEN
          ...
          WHERE category_id = ANY(v_overlapping_ids)
            ...
            AND NOT (menu_item_id::TEXT = ANY(v_trigger_excluded_items))
            AND NOT (menu_item_id::TEXT = ANY(v_reward_excluded_items))
          ORDER BY price ASC -- Cheapest first for free item
          LIMIT 1;
          ...
          IF v_reward_item_price IS NOT NULL THEN
            ...
            -- Consume items (trigger_qty + reward_qty per application)
            DECLARE
            ...
            BEGIN
              FOR v_consume_item IN
                ...
                WHERE category_id = ANY(v_overlapping_ids)
                  ...
                  AND NOT (menu_item_id::TEXT = ANY(v_trigger_excluded_items))
                  AND NOT (menu_item_id::TEXT = ANY(v_reward_excluded_items))
                ORDER BY price ASC -- Consume cheapest first (they're the free ones)
              LOOP
      ...
      ELSE
        ...
        IF v_reward_item_price IS NOT NULL THEN
          ...
          IF v_max_applications > 0 THEN
            ...
            -- Consume trigger items (from most expensive, they're being paid for)
            DECLARE
            ...
            BEGIN
              FOR v_consume_item IN
                ...
                WHERE category_id = ANY(v_trigger_category_ids)
                  AND quantity > 0
                  AND NOT (menu_item_id::TEXT = ANY(v_trigger_excluded_items))
                ORDER BY price DESC -- Expensive triggers (paid items)
              LOOP
            ...
            -- Consume reward items (cheapest ones are free)
            DECLARE
            ...
            BEGIN
              FOR v_consume_item IN
                ...
                WHERE category_id = ANY(v_reward_category_ids)
                  AND quantity > 0
                  AND NOT (menu_item_id::TEXT = ANY(v_reward_excluded_items))
                ORDER BY price ASC -- Cheapest rewards (free items)
              LOOP
