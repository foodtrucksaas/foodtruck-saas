    ...
    CASE v_offer.offer_type
      ...
      WHEN 'buy_x_get_y' THEN
        ...
        IF (v_config->>'type') = 'category_choice' AND v_config->'trigger_category_ids' IS NOT NULL THEN
          ...
          -- Count items from trigger categories
          ...
          IF v_trigger_count >= v_progress_required THEN
            ...
            -- Find the cheapest item from reward categories that's in the cart (or could be added)
            -- For now, calculate based on cheapest available item in reward categories
