-- Fix buy_x_get_y when trigger and reward categories are the SAME (or overlap)
-- Example: "2 pizzas achetées = 1 pizza offerte" - all items are from same category
--
-- Logic change:
-- - If categories overlap: need (trigger_qty + reward_qty) total items from overlapping categories
-- - The cheapest (reward_qty) items become the reward, the rest are triggers
-- - Example: 3 pizzas needed for "2 bought = 1 free" (2 triggers + 1 reward)
...
DECLARE
  ...
  v_overlapping_category_ids UUID[];
  v_total_overlapping_items INTEGER;
  v_reward_quantity INTEGER;
  v_has_category_overlap BOOLEAN;
BEGIN
  ...
  LOOP
    ...
    CASE v_offer.offer_type
      ...
      WHEN 'buy_x_get_y' THEN
        v_reward_quantity := COALESCE((v_config->>'reward_quantity')::INTEGER, 1);
        ...
        IF (v_config->>'type') = 'category_choice' AND v_config->'trigger_category_ids' IS NOT NULL THEN
          ...
          -- Check for category overlap (trigger categories that are also reward categories)
          SELECT ARRAY(
            SELECT unnest(v_trigger_category_ids)
            INTERSECT
            SELECT unnest(v_reward_category_ids)
          ) INTO v_overlapping_category_ids;

          v_has_category_overlap := (array_length(v_overlapping_category_ids, 1) > 0);

          IF v_has_category_overlap THEN
            -- SAME CATEGORY CASE: "2 pizzas = 1 pizza offerte"
            -- Need trigger_qty + reward_qty total items from overlapping categories
            v_progress_required := (v_config->>'trigger_quantity')::INTEGER + v_reward_quantity;

            -- Count ALL eligible items from overlapping categories in cart
            SELECT COALESCE(SUM((item->>'quantity')::INTEGER), 0) INTO v_total_overlapping_items
            FROM jsonb_array_elements(p_cart_items) AS item
            WHERE (item->>'category_id')::UUID = ANY(v_overlapping_category_ids)
            ...
            v_progress_current := v_total_overlapping_items;

            IF v_total_overlapping_items >= v_progress_required THEN
              -- Enough items! The cheapest one(s) become the reward
              -- Find the cheapest item IN THE CART from overlapping categories
              SELECT
              ...
              IF v_cart_reward_item.id IS NOT NULL THEN
                ...
                IF (v_config->>'reward_type') = 'free' THEN
                  v_calculated_discount := v_cart_reward_item.price * v_reward_quantity;
                ELSE
                  v_calculated_discount := COALESCE((v_config->>'reward_value')::INTEGER, 0);
                END IF;
              END IF;
            ELSE
              -- Not enough items - show progress and what they could get
              SELECT mi.name INTO v_free_item_name
              FROM menu_items mi
              WHERE mi.category_id = ANY(v_overlapping_category_ids)
            ...
            END IF;
          ELSE
            -- DIFFERENT CATEGORIES CASE: "2 pizzas = 1 boisson offerte"
            ...
            IF v_trigger_count >= v_progress_required THEN
              -- Find the cheapest reward item IN THE CART
              SELECT
                (item->>'menu_item_id')::UUID as id,
                mi.name,
                (item->>'price')::INTEGER as price
