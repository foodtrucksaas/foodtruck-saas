-- ============================================
-- FIX: Filter offers by days_of_week
-- ============================================
-- Bug: days_of_week was NEVER checked anywhere in the pipeline.
-- An offer configured for "Monday only" would be proposed and validated every day.
--
-- Fix: Add days_of_week filter to process_bundle_offers, process_buy_x_get_y_offers,
-- and the threshold/promo_code loops in get_optimized_offers.
--
-- Testability: A new optional parameter p_check_date allows tests to override NOW().
-- In production, p_check_date is NULL and NOW() AT TIME ZONE 'Europe/Paris' is used.
-- ============================================


-- ============================================
-- HELPER: Process all buy_x_get_y offers (with day_of_week filter)
-- ============================================
DROP FUNCTION IF EXISTS process_buy_x_get_y_offers(UUID, JSONB);
DROP FUNCTION IF EXISTS process_buy_x_get_y_offers(UUID, JSONB, TIMESTAMPTZ);
...
-- ============================================
-- HELPER: Process all bundle offers (with day_of_week filter)
-- ============================================
DROP FUNCTION IF EXISTS process_bundle_offers(UUID, JSONB);
DROP FUNCTION IF EXISTS process_bundle_offers(UUID, JSONB, TIMESTAMPTZ);
...
-- ============================================
-- REPLACE get_optimized_offers (with p_check_date + day_of_week filter)
-- ============================================
DROP FUNCTION IF EXISTS get_optimized_offers(UUID, JSONB, INTEGER, TEXT);
DROP FUNCTION IF EXISTS get_optimized_offers(UUID, JSONB, INTEGER, TEXT, TIMESTAMPTZ);
