-- Fix SELECT INTO RECORD with proper column aliases
-- The previous version used SELECT INTO RECORD without aliases, causing field access issues
...
BEGIN
  ...
  LOOP
    ...
    IF (v_config->>'type') = 'category_choice' AND v_config->'trigger_category_ids' IS NOT NULL THEN
      ...
      -- Get excluded items (default to empty array if null)
      ...
      IF v_has_overlap THEN
        ...
        IF v_max_applications > 0 THEN
          -- Find cheapest item for reward (separate variables, not RECORD)
          SELECT
            (item->>'price')::INTEGER,
            mi.name
      ...
      ELSE
        ...
        -- Find reward items in cart (use separate variables)
        SELECT
          (item->>'price')::INTEGER,
          mi.name,
          (item->>'quantity')::INTEGER
    ...
    ELSE
      ...
      IF v_trigger_count >= v_trigger_qty THEN
        -- Check if reward item is in cart (use separate variables)
