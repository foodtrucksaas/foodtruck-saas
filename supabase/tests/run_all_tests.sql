...
BEGIN
  ...
  v_test_name := 'A05: 3 pizzas (10€, 12€, 15€) = moins chère offerte (10€)';
  v_cart := jsonb_build_array(
    jsonb_build_object('menu_item_id', 'a0000001-0000-0000-0000-000000000001', 'category_id', v_pizza_cat, 'price', 1000, 'quantity', 1, 'name', 'Margherita'),
  ...
  -- Optimal: group by price → 15€ free + 10€ free = 25€
  v_test_name := 'C01: 6 pizzas (3x10€ + 3x15€) = optimal 25€ (15€+10€ free)';
  v_cart := jsonb_build_array(
    jsonb_build_object('menu_item_id', 'a0000001-0000-0000-0000-000000000001', 'category_id', v_pizza_cat, 'price', 1000, 'quantity', 3, 'name', 'Margherita'),
  ...
  v_expected := 2500;  -- Optimal: 15€ + 10€ free by grouping
  SELECT COALESCE(SUM(calculated_discount), 0) INTO v_total_discount
  ...
  -- Optimal: group by price → 12€ free + 10€ free + 8€ free = 30€
  v_test_name := 'C02: 9 pizzas (3x8€ + 3x10€ + 3x12€) = optimal 30€ (12€+10€+8€ free)';
  v_cart := jsonb_build_array(
  ...
  v_expected := 3000;  -- Optimal: 12€ + 10€ + 8€ free by grouping
  SELECT COALESCE(SUM(calculated_discount), 0) INTO v_total_discount
  ...
  v_test_name := 'G04: 2 pizzas (10€+15€) = moins chère offerte (10€)';
  v_cart := jsonb_build_array(
    jsonb_build_object('menu_item_id', 'a0000001-0000-0000-0000-000000000001', 'category_id', v_pizza_cat, 'price', 1000, 'quantity', 1, 'name', 'Margherita'),
...
BEGIN
  ...
  -- ============================================
  v_test_name := 'C03: 4 pizzas + 1 boisson → Bundle + Buy2Get1 (14€)';
  v_cart := jsonb_build_array(
    jsonb_build_object('menu_item_id', 'a0000001-0000-0000-0000-000000000001', 'category_id', v_pizza_cat, 'price', 1000, 'quantity', 1, 'name', 'Margherita'),
  ...
  v_expected := 1400;  -- Bundle(4€) + Buy2Get1(10€) = 14€
  SELECT COALESCE(SUM(calculated_discount), 0) INTO v_total_discount
