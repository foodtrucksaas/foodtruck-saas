...
- [x] **Garde-fou anti-drift pricing** : `packages/shared/src/utils/pricing.ts` (client display) et `supabase/functions/_shared/pricing-engine/resolve-line-items.ts` (`computeUnitPrice`, server authority). Test d'intégration `tests/integration/pricing-coherence.test.ts` valide la cohérence des deux implémentations.
